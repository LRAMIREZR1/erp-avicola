-- Vendedor solo puede editar sus propios pedidos mientras están en estado
-- "pendiente". Apenas el pedido se confirma (o avanza a cualquier otro
-- estado: en preparación, entregado, cancelado), la edición queda
-- reservada para Administrador — porque desde "confirmado" en adelante el
-- pedido ya compromete stock y puede estar en proceso de reparto.
...
create policy "pedidos update" on public.pedidos
  for update using (
    ...
    or (public.rol_actual() = 'vendedor' and vendedor_id = auth.uid() and estado = 'pendiente')
...
begin
  ...
  if public.rol_actual() = 'vendedor' and v_estado <> 'pendiente' then
    raise exception 'Solo puedes editar el pedido mientras está pendiente';
  end if;
